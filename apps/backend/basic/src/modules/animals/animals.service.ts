...
import { callPython } from '@cheol-nx/backend';

@Injectable()
export class AnimalsService {
  async greeting(name: string) {
    const checkedName = await callPython(`hello.py`, name);
    return checkedName;
  }
  ...
  async showAnimalMap() {
    ...
    const data = await callPython(`show_json.py`, JSON.stringify(animalMap));

    return JSON.parse((data as string).replace('\n', ''));
  }
}
